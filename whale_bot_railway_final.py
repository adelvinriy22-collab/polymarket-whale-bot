...
async def main():
    ...
    # 🔑 API Telegram Бота - читається з TELEGRAM_TOKEN змінної
    TELEGRAM_TOKEN = os.getenv('TELEGRAM_TOKEN')
    
    if not TELEGRAM_TOKEN:
        logger.error("❌ ПОМИЛКА: TELEGRAM_TOKEN не встановлений!")
        logger.error("Додай TELEGRAM_TOKEN у змінні середовища Railway")
        raise ValueError("TELEGRAM_TOKEN環境変数が設定されていません")
    
    # 🎯 Фільтри для китів
    MIN_TURNOVER = 10000
    MIN_TRADER_AMOUNT = 5000
    ...
    logger.info(f"📱 Напиши боту в Telegram ID: 8645033199")
